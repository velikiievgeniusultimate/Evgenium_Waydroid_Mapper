...
Version 0.29.0 adds mouse button bindings (including right click and extra side
buttons), configurable MOBA movement by mouse button or WASD, and four selectable
cursor styles in the mapper editor. Profile renaming supports Russian input and
includes on-screen Cyrillic keys for layouts that do not reach the embedded editor.
Movement settings are saved with the profile and retained when a control is moved
through the baggage library.
...
Version 0.20 adds a reversible Android device identity selector under the EWM settings gear. `Настоящий Waydroid` leaves the existing identity alone; `POCO F5 — Mobile Legends` presents Android as the MLBB-compatible POCO F5 (`23049PCD8G`, `marble`) while preserving the real Android version, ABI, fingerprint, Mesa driver and GPU. The helper runs only after Waydroid is fully stopped, saves every pre-EWM value, verifies both `waydroid.cfg` and `waydroid_base.prop`, and restores the exact originals when the native option is selected. System authorization is requested only when a profile actually needs to change; unchanged launches stay prompt-free. Changing the selection while Android is already prepared makes the next one-click start perform the required clean restart.

Version 0.25.2 withdraws the short-lived experimental Google build-identity profile. Pretending that the community Waydroid image is a production POCO build cannot certify Google Play or repair the ARM64-only GAPPS sign-in bug, and the mixed Treble partitions can correctly reject that identity as an inconsistent build. Existing 0.25.0/0.25.1 installations automatically fall back to `Настоящий Waydroid` and restore every property recorded before the experiment. The limited POCO F5 model profile remains available only for Mobile Legends' graphics whitelist.
...
Version 0.25.3 replaces the old cursor-lock approximation with native compositor confinement. F12 toggles gameplay cursor lock. On the host Wayland session EWM uses the native `zwp_pointer_constraints_v1` protocol to make KWin confine the physical pointer to the actual scaled Android picture, not the full host window, so neither letterbox bars nor adjacent monitors can be entered even by a fast mouse flick. The confinement region follows window and fullscreen resizing and is released explicitly by the next F12 press or whenever Integrated Android closes. Qt's ineffective non-popup `setMouseGrabEnabled()` fallback and event-after-the-fact cursor warping are no longer presented as a successful lock. F11 continues to toggle fullscreen. Super/Windows/Meta and any Super chord are consumed only while Integrated Android has focus, before they reach the nested Waydroid compositor; KDE/KWin still receives and handles the physical key normally.

...
Press F6 inside Integrated Android to open the in-game profile manager. It is a movable, resizable panel that remains fully constrained to the game window. Profiles appear as circular avatars: the first name letter is used by default, an imported image can replace it, the active profile always has a green ring, and profiles without a variant for the current Android resolution are marked yellow. The title-bar `×` or F6 closes the manager. The `+` button creates `Пустой профиль N` for the current resolution; the pencil renames non-default profiles, while the gear imports an image and lists every supported resolution.

...
Mapper data is stored in a versioned profile namespace ready for future character presets. The migrated `Default` profile records the Android resolution on which its controls and calibration were created. Selecting another size preserves the profile instead of destroying its calibration and displays a compatibility warning in both the controller and mapper editor. Future profile switching can therefore replace one complete control set atomically while retaining its resolution metadata.
...
`MOBA skill` adds any number of independently bound ability joysticks. Each skill has six settings: exact centre coordinates, joystick diameter, keyboard bind, cast mode, start-speed profile, and perspective calibration. Every speed profile preserves the required ordering — press the exact centre of the small skill button first, then move that same finger towards the cursor — but offers five latency levels: Stable (120 ms), Fast (60 ms), Very fast (30 ms), Instant (10 ms), and Superhuman (the outward MOVE is queued on the very next event-loop turn with no artificial timer delay). Level 4 is the default for existing and new skills. After the initial centre-to-target motion, the skill follows the cursor while held and releases only on the real final key-up event. Keyboard auto-repeat release/press pairs are ignored. If a very short key tap ends during the initial drag, its release is queued until the drag completes. Physical mouse events are consumed while a skill owns the pointer, preventing Waydroid's normal pointer path from cancelling the synthetic touch.
...
Skill calibration is an interactive 24-point measurement (eight directions at 33%, 67%, and 100% joystick distance). After the Start click has completely finished, the mapper presses the ability joystick once and keeps that same Android touch down for the entire wizard. For every sample it slides back to the centre and then smoothly drags outward while the user clicks the actual endpoint shown by the game; the touch releases only after completion or cancellation. While the editor is open, Qt's normal mouse forwarding into the Android surface is disabled so cursor hover cannot cancel or replace the held synthetic finger. Every generated calibration position is also hard-clamped inside both the ability circle and the Android screen. Runtime aiming uses piecewise-affine interpolation over the measured triangular mesh instead of assuming a fixed ellipse; points outside the measured area are clamped to the nearest outer edge. Accidental double clicks are gated while the joystick settles, a Back button repairs the previous sample, and Esc cancels the process while restoring the prior calibration. Moving the character centre, skill centre, or skill diameter deliberately invalidates stale calibration. Changing Android resolution preserves the profile and its calibration but marks the profile as resolution-incompatible until the correct size is selected.
